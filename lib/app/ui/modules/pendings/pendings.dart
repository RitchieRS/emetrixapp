...
class _PendingsPageState extends ConsumerState<PendingsPage> {
                          ...
                          child: Center(
                            child: Container(
                              ...
                              child: ListTile(
                                ...
                                title:
                                    Text(item.tipo ?? '', style: t.mediumDark),
                                subtitle: Text(
                                  _textToTime(DateTime.parse(item.fecha ?? '')),
                                  style: t.text,
                                ),
                              ),
                            ),
  ...
  Future<void> checkNetwork(Pendiente item, int index) async {
    ...
    await sendPending(item, index);
  }

  Future sendPending(Pendiente item, int index) async {
    //post
    final sendPending = await showMessage();
    if (sendPending) {
      showProgress(context: context, title: 'Enviando');
      final result =
          await ref.read(pendingsController.notifier).sendPendings(item);
